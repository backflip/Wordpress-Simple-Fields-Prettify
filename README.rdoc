...
* When used in combination with Custom Field Template (http://wordpress.org/extend/plugins/custom-field-template/), media files are not inserted properly (apparently due to "conflicting" media_send_to_editor hooks). *However*, I just realized that there is absolutely no need of using them both since Simple Fields is capable of showing any number of repeatable field groups on a page. Whoups.
